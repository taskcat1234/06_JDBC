...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		try {
			 ...
			 */
			/* 2-1. Oracle JDBC Driver를 메모리에 Load 하도록 한다*/
			...
			//   DB에서 반환된 결과를 ResultSet 형태로 변환 후 returns
			...
			while(rs.next()) {
				...
				// - 자료형 자리에는 DB 값을 읽어왔을 때
				//  Java에서 저장하기 적절한 자료형을 작성
				String deptID = rs.getString("DEPT_ID");
				String detpTitle = rs.getString("DEPT_TITLE");
				String locationID = rs.getString("LOCATION_ID");
				System.out.printf("부서코드 : %s / 부서명 :  %s / 지역코드 : %s \n"
						,deptID ,detpTitle, locationID
					);
			}
